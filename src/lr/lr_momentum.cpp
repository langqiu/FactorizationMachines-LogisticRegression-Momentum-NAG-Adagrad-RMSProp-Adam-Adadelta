...
namespace lr {
  ...
  void LRMomentum::_backward(const size_t& l, const size_t& r) {
    /*
     ...
     */
#ifdef _DEBUG
    if (_curr_batch == 1) std::cout << "lr momentum backward" << std::endl;
#endif
  ...
  void LRMomentum::_update() {
#ifdef _DEBUG
    if (_curr_batch == 1) std::cout << "lr momentum update" << std::endl;
#endif
    _theta = _theta_new;
