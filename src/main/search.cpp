...
void help_2() {
  std::cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
  std::cout << "please input params separated by comma (iter,batch,fm_dims):" << std::endl;
  std::cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
}
      ...
      for (param_type lambda=v_lambda[1]; lambda<=v_lambda[2]; lambda+=v_lambda[3]) {
        for (param_type beta_1=v_beta_1[1]; beta_1<=v_beta_1[2]; beta_1+=v_beta_1[3]) {
          for (param_type beta_2=v_beta_2[1]; beta_2<=v_beta_2[2]; beta_2+=v_beta_2[3]) {
            lr_instance->init(iter, batch, alpha, lambda, beta_1, beta_2, fm_dims);
            lr_instance->train();
            lr_instance->evaluate();
            score_type logloss = ModelFactory::get_test_dataset()->cal_logloss();
            score_type auc = ModelFactory::get_test_dataset()->cal_auc();
            if (auc_best < auc) {
              auc_best = auc;
              v_alpha[0] = alpha;
              v_lambda[0] = lambda;
